...
var countDoc = FunctionDoc{
	Summary: "Count the number of null / non-null values",
	Description: `By default, only non-null values are counted. This behavior
can be changed through CountOptions to count only nulls or all values
instead.`,
...
func RegisterScalarAggregate(reg FunctionRegistry) {
	countKernels := kernels.GetScalarAggregateKernels()

	fn := NewScalarAggregateFunction("count", Unary(), countDoc)
	fn.SetDefaultOptions(&CountOptions{})
	for _, k := range countKernels {
		if err := fn.AddKernel(k); err != nil {
			panic(err)
		}
	}
	if !reg.AddFunction(fn, false) {
		panic(fmt.Errorf("function 'count' already exists"))
	}
}

// Count returns the number of null / non-null values in values.
//
// The result is a ScalarDatum wrapping an int64 scalar.
